...
--
-- Name: payment_aux_amount; Type: TABLE; Schema: payments; Owner: -
--

CREATE TABLE payments.payment_aux_amount (
    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    payment_id bigint NOT NULL,
    amount numeric(18,2) NOT NULL,
    auxiliar_amount numeric(18,2) NOT NULL,
    currency character varying(3) NOT NULL,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT payment_aux_amount_payment_id_key UNIQUE (payment_id),
    CONSTRAINT payment_aux_amount_payment_id_fkey FOREIGN KEY (payment_id) REFERENCES payments.payment(id) ON DELETE CASCADE
);
